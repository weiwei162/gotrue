...
func (t *Msg91Provider) SendMessage(phone string, message string, channel string) error {
	switch channel {
	case SMSProvider:
...
func (t *Msg91Provider) SendSms(phone string, message string) error {
	body := url.Values{
	...
	if err := json.NewDecoder(res.Body).Decode(&resp); err != nil {
		return fmt.Errorf("msg91 error: failed to parse JSON response body (status code %v): %w", res.StatusCode)
	}
